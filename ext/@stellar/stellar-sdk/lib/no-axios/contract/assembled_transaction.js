...
class AssembledTransaction {
  ...
  static async build(options) {
    ...
    }).addOperation(contract.call(options.method, ...(options.args ?? []))).setTimeout(options.timeoutInSeconds ?? _types.DEFAULT_TIMEOUT);
  ...
  signAuthEntries = async ({
    ...
    for (const [i, entry] of authEntries.entries()) {
      console.log(entry.credentials().switch() !== _stellarBase.xdr.SorobanCredentialsType.sorobanCredentialsAddress());
      ...
      const authEntryAddress = _stellarBase.Address.fromScAddress(entry.credentials().address().address()).toString();
      ...
      const sign = signAuthEntry ?? function fakeSignAuthEntry(x) {
